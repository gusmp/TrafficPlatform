package org.trafficplatform.anprserver.service.plataInfoProviders.vialSecureWs;
...
import org.trafficplatform.anprserver.service.plataInfoProviders.vialSecureWs.beans.VialSecureWsRequest;
import org.trafficplatform.anprserver.service.plataInfoProviders.vialSecureWs.beans.VialSecureWsRequestImage;
import org.trafficplatform.anprserver.service.plataInfoProviders.vialSecureWs.beans.VialSecureWsResponsePlateDetails;
import org.trafficplatform.anprserver.service.plataInfoProviders.vialSecureWs.beans.VialSecureWsResponseRespuesta;
import org.trafficplatform.anprserver.service.plataInfoProviders.vialSecureWs.beans.VialSecureWsResponseRoot;
...
@Service(value = PlateInfoProvider.AnprProviderNames.VIALSECUREWS)
@Slf4j
public class VialSecureWsPlateInfoProvider implements IPlateInfoProvider {

	@Value("${vialSecureWs.endPoint:#{null}}")
	private String endPoint;
	
	@Value("${vialSecureWs.userName:#{null}}")
	private String userName;
	
	@Value("${vialSecureWs.password:#{null}}")
	private String password;
	
	@Value("${vialSecureWs.invoker:#{null}}")
	...
	public List<PlateInfo> getPlateInfo(Resource imageResource) {
		...
		if ((this.endPoint == null) || (this.userName == null) || (this.password == null) || (this.invoker == null)) 
			throw new RuntimeException("The " + PlateInfoProvider.AnprProviderNames.VIALSECUREWS + " plate info provider was not enabled / properly configured");
		
		try {
			HttpHeaders headers = new HttpHeaders();
			headers.setContentType(MediaType.APPLICATION_JSON);
			
			VialSecureWsRequest requestPayload = new VialSecureWsRequest();
			...
			VialSecureWsRequestImage requestImage = new VialSecureWsRequestImage();
			...
			VialSecureWsResponseRoot responseBean = objectMapper.readValue(responseString, VialSecureWsResponseRoot.class);
			
			if (responseBean.isSuccess() == true) {
			
				if (responseBean.getResponse() != null) {
					
					for(VialSecureWsResponseRespuesta respuesta : responseBean.getResponse().getRespuesta()) {
						
						for(VialSecureWsResponsePlateDetails matriculaMetadatos : respuesta.getMatriculaMetadatos()) {
